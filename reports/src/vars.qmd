...
vars <- tibble(var = c("bbl", "rasiarni", "mra", tabvars)) %>%
  mutate(
    order = 1:n(),
...
vars <- vars %>%
  mutate(
    data1 = if_else(str_detect(var, "^shf_") & !var %in% c("shf_age", "shf_sex"), "SwedeHF", NA),
    data2 = case_when(
      str_detect(var, "sos_com_|hosp") & !is.na(Code) ~ paste0("NPR: ", Code),
      str_detect(var, "sos_lm|bbl|rasiarni|mra") ~ paste0("NPDR: ", Code),
    ...
    footnote3 = if_else(str_detect(var, "sos_lm|bbl|rasiarni|mra"), footnote_marker_alphabet(3, format = "latex"), "")
  ) %>%
...
footnotetext <- c(
  ...
  "Medications derived from dispensed prescriptions within 4 months prior to baseline (non-GDMT) and 4 months prior up until day before baseline and 12 months +/- 2 months after baseline respectively (GDMT)."
)
