...
A sensitivity analyses was performed excluding the controls that received a CRT during follow-up. 
...
The association between baseline characteristics and increase in each of the GDMT was evaluated using a multivariable logistic regression 
including the variables shown in the forestplots. The variables were selected based on clinical relevance. The difference between CRT and No CRT was evaluated using an interaction effect (the estimates presented are therefore from different multivariable models).  

### Assumptions

Possible outliers were evaluated using Cook's distance and multicollinearity was 
evaluated using the Variance Inflation Factor (VIF). No action was deemed necessary.  
